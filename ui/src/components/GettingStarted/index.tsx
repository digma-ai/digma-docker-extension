...
export const GettingStarted = (props: GettingStartedProps) => {
  ...
  const javaSampleAppLink = "https://github.com/digma-ai/otel-sample-app-java";

  const handleJavaSampleAppLinkClick = (
    e: React.MouseEvent<HTMLAnchorElement>
  ) => {
    e.preventDefault();
    props.client.host.openExternal(javaSampleAppLink);
  };
...
docker run -rm digmaai/petshop-app-tester:latest
      `}
        ...
        <Link onClick={handleJavaSampleAppLinkClick}>{javaSampleAppLink}</Link>
      </div>
    </s.Container>
  );
};
